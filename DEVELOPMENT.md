...
| `AI_PROVIDER` | — | AI 服务商（deepseek/openai/custom），不配置则 AI 不可用 |
| `AI_BASE_URL` | — | OpenAI 兼容 API 地址 |
| `AI_API_KEY` | — | API Key |
| `AI_MODEL` | — | 模型标识 |
| `AI_REQUEST_TIMEOUT` | `30000` | AI 请求超时（ms） |
...
| `Channel` | 游戏房间 | `surface`, `truth`, `status`, `maxQuestions`, AI 设置字段 |
| `ChannelMember` | 房间成员 | `role` (creator/host/player), `becameHostAt` |
| `Question` | 问题 | `status`, `answer`, `isKeyQuestion`, `isAiAnswered`, `aiReasoning` |
| `Rating` | 评分 | `score` (1-5), `comment` |
| `ChatMessage` | 讨论消息 | `content` (≤500 字), 按 `channelId+createdAt` 索引 |
| `TimelineEvent` | 时间线 | `type` (14 种事件), `metadata` (JSON) |
| `AiHint` | AI 提示 | `content`, `isPublic`, 按 `channelId+userId` 索引 |
...
| POST | `/api/channels/:id/ratings` | 提交评分（所有成员均可） | 是 |
...
| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| GET | `/api/channels/:id/chat?before=ISO&limit=50` | 讨论消息（游标分页） | 是 |
| POST | `/api/channels/:id/chat` | 发送讨论消息 | 是 |

### AI

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| GET | `/api/ai/status` | AI 服务可用性检查 | 是 |
| POST | `/api/channels/:id/hints` | 请求 AI 提示 | 是 |
| GET | `/api/channels/:id/hints?limit=10` | 获取已生成的提示 | 是 |
| PUT | `/api/channels/:id/questions/:qid/correct` | 修正 AI 回答 | 是 |
...
| GET | `/api/users/me/stats` | 个人统计（含四选项分布：yes/no/irrelevant/partial） | 是 |
...
| `question:withdrawn` | 问题已撤回 |
| `question:ai_answered` | AI 自动回答 |
| `question:ai_corrected` | AI 回答被修正 |
| `role:changed` | 身份变更 |
| `chat:new` | 新讨论消息 |
| `hint:shared` | AI 提示被公开 |
| `progress:updated` | 游戏进度更新 |
...
客户端通过 `visibilitychange` 事件监听页面可见性变化。当页面从隐藏恢复为可见时：
- Socket 层自动重连（`socket.ts` 全局监听）
- Channel Socket Hook 重新加入房间并触发 `onVisibilityRestore` 回调
- Channel 数据和讨论消息自动刷新
...
├── QuestionBubble         — 问题气泡（含回答选择器，自己的问题紫色高亮）
├── PlayerInputPanel       — 玩家提问输入（含 AI 线索按钮）
├── DiscussionPanel        — 讨论消息列表（forwardRef 暴露 scrollRef，支持游戏结束分隔线）
├── ChatInput              — 讨论输入框（游戏结束后所有角色可输入）
├── TruthReveal            — 汤底揭晓弹窗
├── StatsModal             — 统计数据弹窗（所有角色均可评分）
├── OnlineUsers            — 在线用户列表
├── ConfirmDialog          — 二次确认对话框
└── 悬浮滚动按钮            — 回顶/到底 FAB（AnimatePresence 动画显隐）
```
...
- 创建者和主理人不能提问
- 回答（状态流转、yes/no/irrelevant/partial 四种类型、关键问题标记）
- 撤回（不计入统计）
- maxQuestions 到达自动结束
- 查看汤底 → 身份转换 + 自动撤回 pending 问题

**rating.test.ts** — 评分系统：
- 评分提交（所有成员均可，包括创建者/主持人/玩家）
- 评分更新
- 评分查询
