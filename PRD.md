...
AI 创作助手帮助用户从零开始构思海龟汤谜题，降低创作门槛。

#### 功能定位

- **不是**：通用聊天机器人
- **而是**：专门的海龟汤创作助手
- **核心价值**：创意激发 + 结构化输出 + 一键应用

#### 入口设计

创作助手入口位于创建海龟汤页面(CreatePage)头部，点击后进入独立的对话页面(路由:`/create/assistant`)。
...
- AI生成的汤面汤底以卡片形式展示
- 支持折叠/展开查看完整内容
- 提供"应用汤面"、"应用汤底"、"应用全部"三个按钮
- 应用后自动填充创建表单，用户可继续编辑

**多方案生成**：
- 用户可要求生成多个不同风格的方案
- 每个方案独立展示，标注风格(如"恐怖向"、"温情向")
- 可选择任一方案应用

#### 模型配置

创作助手支持独立的AI模型配置，与游戏功能分离：

```env
# 创作助手专用 AI 配置（可选）
# 未配置时自动回退到通用 AI_* 配置
CREATION_AI_PROVIDER=openai
CREATION_AI_BASE_URL=https://api.openai.com/v1
CREATION_AI_API_KEY=sk-xxx
CREATION_AI_MODEL=gpt-4-turbo
```

推荐配置：
- **创作助手**：GPT / Claude Sonnet(需要高质量创意生成)
- **AI主持/提示/复盘**：DeepSeek / GPT(成本敏感)

#### 技术实现要点

**后端**：
- `CreationSession` 表存储对话历史(JSON数组)
- SSE流式响应，提升用户体验
- JSON解析提取生成的汤面汤底

**前端**：
- React Markdown渲染AI回复(支持Markdown格式)
- Zustand管理对话状态
- 自动应用生成内容到创建表单
...
|------|------|------|
| id | UUID | 主键 |
| user_id | UUID | 用户ID，唯一约束 |
| messages | JSON | 对话历史，JSON数组 |
| created_at | TIMESTAMP | 创建时间 |
...
### 10.8 创作助手（需配置 AI 环境变量）

```
GET                       /api/creation-sessions                    获取当前Session
POST                      /api/creation-sessions/chat/stream        发送消息（SSE流式）
DELETE                    /api/creation-sessions                    清空Session
```
...
# 创作助手专用 AI 配置（可选）
# 未配置时自动回退到上方通用 AI_* 配置
# 适用场景：创作助手使用更高质量的模型（如 GPT-4），游戏功能使用经济型模型
CREATION_AI_PROVIDER=
